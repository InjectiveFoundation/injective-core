...
# Gethlocal RPC URL - can be overridden via environment variable
GETHLOCAL_RPC_URL="${GETHLOCAL_RPC_URL:-http://host.docker.internal:8989}"
...
docker run --rm \
  ...
  -e HYP_LOG_LEVEL=debug \
	...
	-e HYP_RELAYCHAINS=injectivez,gethlocal \
  ...
  -e HYP_CHAINS_INJECTIVEZ_GRPCURLS_0_HTTP=http://host.docker.internal:9900 \
  	-e HYP_CHAINS_GETHLOCAL_INDEX_CHUNK=20 \
  	-e HYP_CHAINS_GETHLOCAL_NAME=gethlocal \
  	-e HYP_CHAINS_GETHLOCAL_SIGNER_TYPE=hexKey \
  	-e HYP_CHAINS_GETHLOCAL_SIGNER_KEY=$VALIDATOR_PK \
  	-e HYP_CHAINS_GETHLOCAL_SIGNER_ACCOUNTADDRESSTYPE=Ethereum \
  	-e HYP_CHAINS_GETHLOCAL_CONTRACTADDRESSBYTES=20 \
  	-e HYP_CHAINS_GETHLOCAL_NATIVETOKEN_DECIMALS=18 \
  	-e HYP_CHAINS_GETHLOCAL_NATIVETOKEN_DENOM=ether \
  	-e HYP_CHAINS_GETHLOCAL_MERKLETREEHOOK=0xf96973FbAfC2ba480A99DA0c4d9c6BddDc5e0477 \
  	-e HYP_CHAINS_GETHLOCAL_VALIDATORANNOUNCE=0x8c48be68Dd10c9BdF5edB716D5dB400152af397C \
  	-e HYP_CHAINS_GETHLOCAL_CHAINID=1337 \
  	-e HYP_CHAINS_GETHLOCAL_DOMAINID=1337 \
  	-e HYP_CHAINS_GETHLOCAL_PROTOCOL=ethereum \
  	-e HYP_CHAINS_GETHLOCAL_MAILBOX=0x067A3faF94e52886bEBe17665aA9C395A8cB08Dd \
  	-e HYP_CHAINS_GETHLOCAL_INTERCHAINGASPAYMASTER=0x3B1b6F1309c6Dfb14AB093F9a5C0eFF5903D7516 \
    -e HYP_CHAINS_GETHLOCAL_RPCURLS_0_HTTP=$GETHLOCAL_RPC_URL \
  -v $(pwd)/$VALIDATOR_SIG_DIR:/validator-sig \
  public.ecr.aws/l9h3g6c6/injective-hyperlane-agents:v1.16.0-inj \
  bash -c "mkdir -p /tmp/relayer-db && ./relayer"
