...
vote() {
  ...
  --chain-id injective-1 --broadcast-mode block
}


CW20_CODE_ID=1
echo "Store Wasm CW20 Code..."
yes $PASSPHRASE | injectived tx wasm store ./scripts/wasm-contracts/cw20_base.wasm --from=wasm --chain-id=$CHAINID --broadcast-mode=block --gas=3000000 --fees=1500000000000000inj --yes
sleep 3
...
yes $PASSPHRASE | injectived tx wasm store ./scripts/wasm-contracts/cw20_adapter.wasm --from=wasm --chain-id=$CHAINID --broadcast-mode=block --fees=1500000000000000inj --gas=3000000 --yes
sleep 3
...
yes $PASSPHRASE | injectived tx bank send wasm $ADAPTER_ADDRESS 100000000000000000000inj --from wasm --chain-id=$CHAINID --broadcast-mode=block --gas=3000000 --fees=1500000000000000inj --yes
sleep 3
...
--from wasm --chain-id=injective-1 --fees=1500000000000000inj --yes --broadcast-mode=block
sleep 3
...
PROPOSAL_ID=1
yes 12345678 | injectived tx gov submit-proposal wasm-store ./scripts/wasm-contracts/cw20_base.wasm \
