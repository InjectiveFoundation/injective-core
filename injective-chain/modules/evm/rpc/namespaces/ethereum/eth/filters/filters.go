...
import (
	...
	backendtypes "github.com/InjectiveLabs/injective-core/injective-chain/modules/evm/rpc/backend"
...
func NewBlockFilter(logger log.Logger, backend Backend, criteria ethfilters.FilterCriteria) *Filter {
...
func newFilter(logger log.Logger, backend Backend, criteria ethfilters.FilterCriteria, bloomFilters [][]BloomIV) *Filter {
	return &Filter{
		logger:       logger,
		backend:      backend,
		criteria:     criteria,
...
func (f *Filter) blockLogs(blockRes *cmrpctypes.ResultBlockResults, bloom ethtypes.Bloom) ([]*ethtypes.Log, error) {
	if !bloomFilter(bloom, f.criteria.Addresses, f.criteria.Topics) {
		return []*ethtypes.Log{}, nil
	}

	logsList, err := backendtypes.GetLogsFromBlockResults(blockRes)
	if err != nil {
