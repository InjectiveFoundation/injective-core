...
function checkTestEnv() {
  ...
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network injective', 'run all tests using injective network')
    .example('$0 --network injective --allowTests=test1,test2', 'run only test1 and test2 using injective network')
    .help('h').alias('h', 'help')
    .describe('network', 'set which network to use: ganache|injective')
    ...
    .boolean('verbose-log').describe('verbose-log', 'print injectived output, default false')
    .argv;
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'injective' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or injective');
    }
    else {
...
function loadTests(runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach(dirname => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-injective'];
...
function performTestSuite({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-injective';
...
function setupNetwork({ runConfig, timeout }) {
  if (runConfig.network !== 'injective') {
    // no need to start ganache. Truffle will start it
    return;
  }

  // Spawn the injective process

  const spawnPromise = new Promise((resolve, reject) => {
    const injectivedProc = spawn('./init-test-node.sh', {
      cwd: __dirname,
    ...
    if (runConfig.verboseLog) {
      injectivedProc.stdout.pipe(process.stdout);
    }
    injectivedProc.stderr.on('data', d => {
      ...
      if (oLine.indexOf('Starting JSON-RPC server') !== -1) {
        ...
        resolve(injectivedProc);
      }
    });
  });

  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start injectived timeout!')), timeout);
  });
