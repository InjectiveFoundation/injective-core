...
func SimulateEthTx(
	...
	if errors.Is(err, ErrNoEnoughBalance) {
	...
	txConfig := MakeSimEncodingConfig(nil).TxConfig
...
func CreateRandomValidEthTx(ctx *simulateContext,
	...
	ethChainID := ctx.keeper.EIP155ChainID(ctx.context)
...
func GetSignedTx(
	...
	if err := msg.Sign(ethtypes.LatestSignerForChainID(ctx.keeper.EIP155ChainID(ctx.context)), newSigner(prv)); err != nil {
		return nil, err
	}
