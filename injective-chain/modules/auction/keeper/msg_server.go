...
func (k msgServer) Bid(goCtx context.Context, msg *types.MsgBid) (*types.MsgBidResponse, error) {
	...
	if msg.Round != round {
		...
		return nil, sdkerrors.Wrapf(types.ErrBidRound, "current round is %d but got bid for %d", round, msg.Round)
	}
	// check valid bid
	lastBid := k.GetHighestBid(ctx)
	if msg.BidAmount.Amount.LT(lastBid.Amount.Amount) {
		metrics.ReportFuncError(k.svcTags)
		return nil, sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "Bid must exceed current highest bid")
	}
	...
	if lastBid.Amount.Amount.ToDec().Mul(sdk.OneDec().Add(params.MinNextBidIncrementRate)).GT(msg.BidAmount.Amount.ToDec()) {
		metrics.ReportFuncError(k.svcTags)
		return nil, sdkerrors.Wrapf(sdkerrors.ErrInvalidRequest, "new bid should be bigger than last bid + min increment percentage")
	}
	...
	if err != nil {
		metrics.ReportFuncError(k.svcTags)
		return nil, sdkerrors.Wrap(err, "invalid sender address")
	}
