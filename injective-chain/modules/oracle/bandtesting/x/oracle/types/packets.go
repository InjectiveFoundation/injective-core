...
func (p OracleRequestPacketData) ValidateBasic() error {
	if p.MinCount <= 0 {
		return sdkerrors.Wrapf(bandoracletypes.ErrInvalidMinCount, "got: %d", p.MinCount)
	}
	if p.AskCount < p.MinCount {
		return sdkerrors.Wrapf(bandoracletypes.ErrInvalidAskCount, "got: %d, min count: %d", p.AskCount, p.MinCount)
	}
	...
	if p.PrepareGas <= 0 {
		return sdkerrors.Wrapf(bandoracletypes.ErrInvalidOwasmGas, "invalid prepare gas: %d", p.PrepareGas)
	}
	if p.ExecuteGas <= 0 {
		return sdkerrors.Wrapf(bandoracletypes.ErrInvalidOwasmGas, "invalid execute gas: %d", p.ExecuteGas)
	}
	if p.PrepareGas+p.ExecuteGas > MaximumOwasmGas {
		return sdkerrors.Wrapf(bandoracletypes.ErrInvalidOwasmGas, "sum of prepare gas and execute gas (%d) exceed %d", p.PrepareGas+p.ExecuteGas, MaximumOwasmGas)
	}
